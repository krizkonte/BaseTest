...
function BordasSection() {
  ...
  const [customBorders, setCustomBorders] = React.useState(() => {
    ...
    return {
      "--box-rounded": 3, // índice para --radius-4xl
  ...
  const captureOriginalValues = React.useCallback(() => {
    ...
    ROUNDED_VARS.forEach(({ var: cssVar }) => {
      ...
      // Encontra o índice correspondente no RADIUS_STEPS
      const stepIndex = RADIUS_STEPS.findIndex(
  ...
  React.useEffect(() => {
    ...
    if (brand === "custom") {
      ...
      Object.entries(customBorders).forEach(([cssVar, stepIndex]) => {
        const step = RADIUS_STEPS[stepIndex as number];
        if (step) {
  ...
  // Função para obter o valor atual (index do RADIUS_STEPS)
  ...
  return (
    <section className="mb-6">
      ...
      {ROUNDED_VARS.map(({ var: cssVar, label }) => {
        ...
        const currentStep = RADIUS_STEPS[currentIdx] || RADIUS_STEPS[0];
          ...
          <div key={cssVar} className="mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs w-20 text-right">
                {currentStep.label} ({currentStep.value}rem)
              </span>
            </div>
          </div>
        );
